...
DATA_FILE = APP_FOLDER / "studypilot_bridge_data.json"


def print_banner():
    print("\nStudyPilot")
    print(
        "Bridge milestone: menu-driven console behavior between Week 6 and the final app."
    )
...
def log_session_flow(data):
    ...
    session_date = prompt_date(
        "Session date (YYYY-MM-DD, press Enter for today)", allow_blank=True
    )
    if not session_date:
...
def show_main_menu():
    ...
    print("6. View progress summary and recommendations")
    print("0. Exit")
