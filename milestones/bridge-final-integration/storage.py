...
def normalize_data(data):
    ...
    if isinstance(next_task_id, int) and next_task_id > 0:
        ...
        cleaned_data["next_task_id"] = next_id(cleaned_data["tasks"])
    ...
    if isinstance(next_session_id, int) and next_session_id > 0:
        ...
        cleaned_data["next_session_id"] = next_id(cleaned_data["sessions"])
...
def load_data(file_path):
    ...
    if not path.exists():
        return (
            default_data(),
            f"No existing bridge data file found. Starting with a fresh file named {path.name}.",
        )
    ...
    try:
        ...
        return (
            default_data(),
            "The bridge JSON data file could not be read. StudyPilot is starting with empty data.",
        )
    except OSError as error:
        return default_data(), f"The bridge data file could not be opened: {error}"

    return normalize_data(data), f"Loaded bridge data from {path.name}."
...
def save_data(file_path, data):
    ...
    try:
        ...
        return False, f"StudyPilot could not save the bridge JSON data file: {error}"

    return True, f"Saved bridge data to {path.name}."
